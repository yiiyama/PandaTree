...
panda::PJet::datastore::allocate(UInt_t _nmax)
{
  ...
  id = new UInt_t[nmax_];
  nConstituents = new UInt_t[nmax_];
  constituents_ = new UInt_t[nmax_][128];
}
...
{
  UInt_t (&constituentsIndices)[128](gStore.getData(this).constituents_[0]);
  for (UInt_t i0(0); i0 != sizeof(constituentsIndices); ++i0) {
    constituents[i0].setIndex(constituentsIndices[i0]);
  }
}
...
{
  UInt_t (&constituentsIndices)[128](_data.constituents_[_idx]);
  for (UInt_t i0(0); i0 != sizeof(constituentsIndices); ++i0) {
    constituents[i0].setIndex(constituentsIndices[i0]);
  }
}
...
panda::PJet::setAddress(TTree& _tree, utils::BranchList const& _branches/* = {"*"}*/, Bool_t _setStatus/* = kTRUE*/)
{
  ...
  utils::setAddress(_tree, name, "nConstituents", &nConstituents, _branches, _setStatus);
  utils::setAddress(_tree, name, "constituents_", &constituents[0].idx(), _branches, true);
}
